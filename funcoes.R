...
resumir_variavel_resposta = function(dados = dados_ano_final, var_resposta = "Cash", var_group = "Lever") {
  ...
  call = substitute(
    ...
    %>% dplyr::summarise(
                  ...
                  Percentil50Var = quantile(VarResposta, probs = c(0.5), na.rm = TRUE),
                  ...
                  Percentil50Regret = quantile(VarRegret, probs = c(0.25), na.rm = TRUE),
...
#' @export
obter_df_vulnerabilidade = function(results, estrategia_candidata, variavel_resposta = "sNPVProfit1RegretPerc" , threshold = 0.1, planilha_inputs, sentido_vulnerabilidade = ">=", AnaliseRegret) {
  
  # Se não foi informada uma analise regret específica, utilizar a do Results.
  if(!exists(x = "AnaliseRegret")){
    AnaliseRegret = results$AnaliseRegret
  }
  
  if(sentido_vulnerabilidade == ">=") {
    AnaliseRegret$Dados$CasoInteresse = as.numeric(AnaliseRegret$Dados[,variavel_resposta] >= threshold)  
  } else {
    AnaliseRegret$Dados$CasoInteresse = as.numeric(AnaliseRegret$Dados[,variavel_resposta] <= threshold)  
  }
  
  # Obter Ensemble com Dados Simulados:
  ensemble_e_resultados = dplyr::inner_join(as.data.frame(results$Ensemble), AnaliseRegret$Dados, by = "Scenario")
